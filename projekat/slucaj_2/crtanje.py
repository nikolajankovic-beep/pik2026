...
podaci = np.loadtxt('najgori_odziv_2.txt') 
...
ax.text((tr_10 + tr_90)/2, visina_kote + 0.08, r'$t_r = 46.6$ ps', 
...
# Tekst (vrednost vučemo iz tvoje tabele za odnos 2.0)
ax.text((tf_90 + tf_10)/2, visina_kote + 0.08, r'$t_f = 52.1$ ps', 
        ha='center', va='bottom', fontsize=12, fontweight='bold', bbox=dict(facecolor='white', edgecolor='none', alpha=0.8))

# 5. Ulepšavanje
ax.set_title('Tranzijentni odziv: Vremena rasta i pada (Odnos $W_p / W_n = 2.0$)', fontsize=14, fontweight='bold', pad=15)
...
# 6. Čuvanje
plt.tight_layout()
plt.savefig('odziv_2.png', dpi=300, bbox_inches='tight')
plt.show()
