...
podaci = np.loadtxt('/home/Nikola.Jankovic/projekat/slucaj_2/python/odziv_2_matching.txt') 
...
# ---> IZRAČUNAVANJE VREMENA (dodano) <---
t_r_izracunato = tr_90 - tr_10
t_f_izracunato = tf_10 - tf_90
...
# Tekst (Sada koristi prave izračunate vrijednosti)
ax.text((tr_10 + tr_90)/2, visina_kote + 0.08, f'$t_r = {t_r_izracunato:.1f}$ ps', 
...
# Tekst (Sada koristi prave izračunate vrijednosti)
ax.text((tf_90 + tf_10)/2, visina_kote + 0.08, f'$t_f = {t_f_izracunato:.1f}$ ps', 
        ha='center', va='bottom', fontsize=12, fontweight='bold', bbox=dict(facecolor='white', edgecolor='none', alpha=0.8))

# 5. Ulepšavanje
ax.set_title('Tranzijentna analiza: Vremena rasta i pada (Odnos $W_p / W_n = 2.0$)', fontsize=14, fontweight='bold', pad=15)
...
# 6. Čuvanje
plt.tight_layout()
plt.savefig('odziv_2_matching.svg', format='svg' , bbox_inches='tight')
plt.show()
