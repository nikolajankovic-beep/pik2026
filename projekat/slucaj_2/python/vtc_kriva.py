...
# 3. Fiksirano traženje tačke T (VT)
V_prava = -Va + Voh + Vol
# Tražimo presek ISKLJUČIVO u zabranjenoj zoni da izbegnemo ivice grafika
maska = (Va > Vil) & (Va < Vih)
indeks_vt = np.argmin(np.abs(Vout[maska] - V_prava[maska]))
# Vraćamo stvarni indeks iz originalnog niza
stvarn_indeks_vt = np.where(maska)[0][indeks_vt] 
Vt = Va[stvarn_indeks_vt]
Vt_out = Vout[stvarn_indeks_vt]

# ==========================================
# 4. PROFESIONALNO CRTANJE GRAFIKA
# ==========================================
plt.figure(figsize=(12, 8)) # Veća slika za bolju preglednost
...
# Tačka T (Prag preklapanja)
plt.plot(Vt, Vt_out, 'ro', markersize=6, zorder=5) # Crvena tačka da se istakne
...
# --- STRELICE I TEKST NA OSAMA ---

# Margine šuma (NML i NMH)
...
# Legenda premeštena gore desno gde ima mesta
