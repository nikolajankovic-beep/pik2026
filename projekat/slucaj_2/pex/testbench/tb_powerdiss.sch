...
Power dissipation} 190 -670 0 0 0.4 0.4 {}
N 290 -500 520 -500 {
lab=a}
N 290 -500 290 -480 {
lab=a}
...
N 680 -410 690 -410 {
lab=GND}
C {vsource.sym} 290 -450 0 0 {name=Va value="PULSE(0 1.5 0 20p 20p 200p 500p)" savecurrent=false}
...
.include /home/Nikola.Jankovic/projekat/slucaj_2/pex/funkcija_matching.spice
.tran 0.1p 500p
.control
  save all
  run
  ...
  * 1. STATIČKA SNAGA 
  meas tran i_leak FIND i(Vdd) AT=450p
  let p_stat = abs(i_leak * 1.5)

  * 2. DINAMIČKA SNAGA 
  meas tran i_total_avg AVG i(Vdd) from=0 to=500p
  let p_dyn = abs((i_total_avg - i_leak) * 1.5)
...
C {/home/Nikola.Jankovic/projekat/slucaj_2/pex/funkcija_matching.sym} 670 -470 0 0 {name=x1}
