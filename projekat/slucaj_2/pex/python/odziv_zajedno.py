...
# --- FUNKCIJA ZA AUTOMATSKO NALAŽENJE PRESEKA ---
...
# 1. Učitavanje podataka za OBA slučaja
# ZAMENI 'odziv_unmatched.txt' sa tvojim stvarnim imenom fajla za prvu simulaciju!
podaci_u = np.loadtxt('odziv_nonmatch.txt') 
t_u = podaci_u[:, 0] * 1e12  
v_in = podaci_u[:, 1]        
v_out_u = podaci_u[:, 3]     

podaci_m = np.loadtxt('odziv_match.txt') 
t_m = podaci_m[:, 0] * 1e12  
# Ulaz (v_in_m) preskačemo jer pretpostavljamo da je pobuda potpuno ista
v_out_m = podaci_m[:, 3]     

# 2. Računanje vremena za UNMATCHED
...
# 3. Računanje vremena za MATCHED
...
# Legendu spuštamo ISPOD grafika da ne bi smetala signalima unutar samog okvira
ax.legend(loc='upper center', bbox_to_anchor=(0.5, -0.12), ncol=3, fontsize=11, frameon=True, edgecolor='black', framealpha=0.9)

plt.tight_layout()
plt.savefig('odziv_zajedno.svg', format='svg', bbox_inches='tight')
plt.show()
